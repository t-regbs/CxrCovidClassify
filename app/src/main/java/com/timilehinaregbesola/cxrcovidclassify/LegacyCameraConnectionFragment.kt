...
import com.timilehinaregbesola.cxrcovidclassify.databinding.FragmentCameraConnnectionBinding
...
class LegacyCameraConnectionFragment : Fragment {
    ...
    private lateinit var binding: FragmentCameraConnnectionBinding
    private lateinit var camera: Camera
    private var imageListener: PreviewCallback? = null
    private var desiredSize: Size? = null

    /**
     * [binding.texture.SurfaceTextureListener] handles several lifecycle events on a [ ].
     */
            ...
            camera.addCallbackBuffer(
                ByteArray(
                    ImageUtils.getYUVByteSize(
                        /* width= */s.height, /* height= */
                        s.width
                    )
                )
            )
            binding.texture.setAspectRatio( /* width= */s.height, /* height= */s.width)
            camera.startPreview()
        }
    ...
    override fun onCreateView(
        ...
        binding = FragmentCameraConnnectionBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onResume() {
        ...
        if (binding.texture.isAvailable()) {
            ...
            binding.texture.setSurfaceTextureListener(surfaceTextureListener)
        }
    }
